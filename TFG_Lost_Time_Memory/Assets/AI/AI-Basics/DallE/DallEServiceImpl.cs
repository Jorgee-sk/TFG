...
public class DallEServiceImpl : MonoBehaviour
{
    ...
    private async Task<T> ExecuteRequest<T>(string path, string method, byte[] payload = null) where T : IBaseResponse
    {
        ...
        using (var request = UnityWebRequest.Put(path, payload))
        {
            ...
            if ((PlayerPrefs.GetString("organization") == null ||
                 PlayerPrefs.GetString("organization").Equals(String.Empty)) &&
                (PlayerPrefs.GetString("private_api_key") == null ||
                 PlayerPrefs.GetString("private_api_key").Equals(String.Empty)))
            {
                auth = new Auth();
            }
            else if ((PlayerPrefs.GetString("organization") == null ||
                      PlayerPrefs.GetString("organization").Equals(String.Empty)) &&
                     (PlayerPrefs.GetString("private_api_key") != null ||
                      !PlayerPrefs.GetString("private_api_key").Equals(String.Empty)))
            {
